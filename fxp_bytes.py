...
def deserialize_address(b: bytes):
    """
...
def serialize_utcdatetime(utc: datetime) -> bytes:
    ...
    A 64-bit integer number of microseconds that have passed since 00:00:00 UTC on 
    1 January 1970
...
def marshal_message(quote_sequence) -> bytes:
    ...
    >>> len(b)  # each record is 32 bytes, so 2 of those in this test 64
